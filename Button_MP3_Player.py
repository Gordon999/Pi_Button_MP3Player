    ...
    if X == -1 and coin_det == 1:
        for A in range(0,int(len(pins)/2)):
            if GPIO.input(pins[A])== 0:
                X = A
                start = time.time()
                print ("Letter",letters[X],"pressed")
                time_start = time.time()
                if X == 1:
                    B_pressed = 1
        ...
        while Con_Play == 1 :
            ...
            while poll == None:                         # track still playing
                ...
                if  GPIO.input(pins[0]) == 0:           # Previous track     (A pressed)
                    ...
                    skip = 1
                    Z += 1
                elif  GPIO.input(pins[3]) == 0:           # 10 Track Backwards (D pressed)
                    ...
                    if Z < -1:
                        Z = len(tracks) + Z
                elif  GPIO.input(pins[4]) == 0:           # 10 Track Forward   (E pressed)
                    print ("Skip 10 Track forward...")
                    time.sleep(0.5)
                    os.killpg(p.pid, signal.SIGTERM)
                    skip = 1
                    Z += 10
                elif  GPIO.input(stop) == 0:              # Exiting Continuous Play (if STOP pressed > 5 secs)
                    timer1 = time.time()
                ...
                elif  GPIO.input(shutdown) == 0:          # shutdown
